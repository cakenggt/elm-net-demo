...
generateNodeCircles net width height =
    let
        ...
        circleGenerator =
            generateSingleNodeCircle nodeWidth
    in
...
getNodeWidth net width =
    let
        maxNodes =
            Maybe.withDefault 1 (List.maximum [ net.inputs, net.hidden, net.outputs, 3 ])
    in
...
generateNodePositions net width height =
    let
        ...
        centerBoxWidth =
            toFloat width - (2 * padding)

        centerBoxHeight =
            toFloat height - (2 * padding)
    in
    List.concat
        [ List.map
            (\i ->
                generateSingleNodePosition padding (1.5 * nodeWidth) centerBoxHeight i net.inputs
            )
            (List.range 0 (net.inputs - 1))
        , List.map
            (\i ->
                generateSingleNodePosition padding (3.5 * nodeWidth) centerBoxHeight i net.hidden
            )
            (List.range 0 (net.hidden - 1))
        , List.map
            (\i ->
                generateSingleNodePosition padding (5.5 * nodeWidth) centerBoxHeight i net.outputs
            )
            (List.range 0 (net.outputs - 1))
        ]
...
generateSingleNodePosition padding xpos centerBoxHeight index total =
    let
        ...
        vertSpacing =
            centerBoxHeight / toFloat total

        centerPadding =
            vertSpacing / 2

        ypos =
            padding + centerPadding + (ifloat * vertSpacing)
    in
    ( xpos, ypos )
...
generateWeightLines net width height =
    let
        inputWeights =
            List.take (net.inputs * net.hidden) net.weights

        outputWeights =
            List.drop (net.inputs * net.hidden) net.weights

        nodePositions =
            generateNodePositions net width height

        inputs =
            List.take net.inputs nodePositions

        hiddens =
            List.take net.hidden (List.drop net.inputs nodePositions)

        outputs =
            List.drop (net.inputs + net.hidden) nodePositions
